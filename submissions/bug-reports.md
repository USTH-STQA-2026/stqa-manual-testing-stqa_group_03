...
**Bug ID:** BUG-REQ02-01 (Tham chiếu: TC-04)
...
## 3. BUG-REQ03-01: Hệ thống tìm kiếm lỗi khi từ khóa chứa khoảng trắng ở đầu hoặc cuối

**Bug ID:** BUG-REQ03-01 (Tham chiếu: TC-07)

**Tiêu đề:** Hệ thống báo lỗi "Không tìm thấy sách" khi nhập từ khóa tìm kiếm có chứa khoảng trắng thừa ở đầu hoặc cuối chuỗi (Thiếu logic Trim Space)
...
- Người dùng đã đăng nhập thành công vào hệ thống bằng một tài khoản Thành viên hợp lệ.
- Đang ở giao diện trang chủ hiển thị danh sách sách.

**Bước tái hiện:**

1. Di chuyển chuột đến ô nhập liệu tìm kiếm sách.
2. Nhập vào chuỗi từ khóa có chứa ký tự khoảng trắng ở cả hai đầu: `" Flutter "` _(có dấu cách phía trước và dấu cách phía sau chữ Flutter)_.
3. Nhấn phím Enter hoặc nhấp vào biểu tượng kính lúp để thực thi truy vấn tìm kiếm.

**Kết quả mong đợi:**
Hệ thống phải tự động loại bỏ (trim) các ký tự khoảng trắng dư thừa ở đầu và cuối chuỗi ký tự trước khi thực hiện truy vấn, nhận diện từ khóa chính là `Flutter` và hiển thị chính xác cuốn sách "Lập trình Flutter cơ bản".

**Kết quả thực tế:**
Hệ thống không xử lý chuẩn hóa chuỗi đầu vào mà gộp luôn dấu cách vào từ khóa tìm kiếm chính xác. Màn hình trả về kết quả trống kèm thông báo lỗi `"Không tìm thấy sách"`, mặc dù cuốn sách "Lập trình Flutter cơ bản" đang tồn tại.

**Tác động:**
Làm giảm nghiêm trọng trải nghiệm người dùng (UX) do thói quen sao chép/nhập liệu từ khóa của người dùng thường vô tình kéo theo khoảng trắng. Hệ thống hoạt động quá cứng nhắc, khiến người dùng lầm tưởng sách không tồn tại trong thư viện.

**Mức độ:**
**Trung bình (Medium)** — Lỗi xử lý định dạng dữ liệu đầu vào (Validation & Data Formatting Issue). Chức năng cốt lõi không bị hỏng hoàn toàn nhưng hoạt động sai lệch khi dữ liệu thô chưa được chuẩn hóa.

**Minh chứng:**

- Ảnh chụp màn hình đính kèm: `evidence/bug_11.png` _(Hiển thị ô tìm kiếm có khoảng trắng và thông báo không tìm thấy sách)_.

**Đề xuất xử lý:**
Tại hàm xử lý sự kiện tìm kiếm ở Frontend (hoặc tại API Controller ở Backend), thực hiện gọi phương thức `.trim()` đối với chuỗi giá trị lấy từ ô input (Ví dụ: `searchKeyword.trim()`) trước khi gửi tham số đi lọc dữ liệu.

---

## 4. BUG-REQ03-02: Ô tìm kiếm ghi đè và bỏ qua hoàn toàn điều kiện lọc của Thể loại sách

**Bug ID:** BUG-REQ03-02 (Tham chiếu: TC-09)
...
**Bug ID:** BUG-REQ04-01 (Tham chiếu: TC-16)
...
**Bug ID:** BUG-REQ05-01 (Tham chiếu: TC-05-05)
